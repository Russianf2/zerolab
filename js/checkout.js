      ...
      ${cart.map(item => `
        <div style="display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--border);font-size:0.85rem">
          <div>
            ...
            <div style="color:var(--text-muted);font-size:0.75rem">${item.size || ''}${item.color ? ' / ' + item.color : ''} × ${item.quantity} · ${(item.weight || 0)} KG</div>
          </div>
          <div style="font-weight:500">${formatCOP(item.price * item.quantity)}</div>
        </div>
      `).join('')}

      <div class="summary-row" style="margin-top:16px"><span>Peso Total</span><span>${cart.reduce((s,i) => s + (i.weight || 0) * i.quantity, 0).toFixed(2)} KG</span></div>
