...
function renderReceipt() {
  ...
  document.getElementById('receiptItems').innerHTML = o.items.map(item => `
    <div class="receipt-item">
      <div class="receipt-item-info">
        ...
        <span class="receipt-item-detail">${item.size || ''}${item.color ? ' — ' + item.color : ''} × ${item.quantity} · ${(item.weight || 0)} KG c/u</span>
      </div>
  ...
  document.getElementById('receiptTotals').innerHTML = `
    <div class="receipt-total-row"><span>Peso Total</span><span>${o.items.reduce((s,i) => s + (i.weight || 0) * i.quantity, 0).toFixed(2)} KG</span></div>
